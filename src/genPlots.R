genPlots <- function(dataframe, parameter, geometry, geoid = NULL){
  ...
  if(parameter == "Puntaje de Colorado EnviroScreen"){
    # font for title  
    fontHeader <- list(
      family = "museo-sans",
      color = "#000000",
  ...
  if(parameter %in% c("Poblaciones sensibles")){
    xlabel <- "Susceptibilidad"
  }
  if(parameter %in% c("Características demográficas")){
    xlabel <- "Vulnerabilidad"
  }
  if(parameter %in% c("Puntaje de Colorado EnviroScreen",
                      "Exposiciones ambientales",
                      "Efectos ambientales",
                      "Vulnerabilidad climátic")){
    xlabel <- "Carga"
  }
